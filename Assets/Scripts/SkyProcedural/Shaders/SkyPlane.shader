Shader "Custom/SkyPlane" {
    ...
    SubShader {
        // Plane sits in the background; ZWrite Off so sprites render on top
        ...
        Pass {
            ...
            // -----------------------------------------
            // Functions  (identical to LowPolySkybox)
            // -----------------------------------------
                ...
                half4 CelestialColor(float3 position, sampler2D tex, fixed size, half4 tint, fixed haloCoef) {
                    fixed depthCheck = step(position.z, 0);
            ...
            v2f vert(appdata v)
            {
                ...
                // Derive a view direction from UV so the rest of the shader
                // works exactly like the skybox version.
                // UV (0,0)=bottom-left, (1,1)=top-right → direction (-1..1, -1..1, -1)
                float3 dir = normalize(float3(v.uv.x * 2.0 - 1.0,
                                              v.uv.y * 2.0 - 1.0,
                                              -1.0));
                OUT.vertex = dir;

                #if !SUN_OFF
                    OUT.sunPosition = mul(sunMatrix, float4(dir, 0.0));
                #endif
                #if !MOON_OFF
                    OUT.moonPosition = mul(moonMatrix, float4(dir, 0.0));
                #endif
                #if !CLOUDS_OFF
                    OUT.cloudsPosition = RotateAroundYInDegrees(float4(dir, 1.0), _CloudsRotation);
                    OUT.cloudsPosition.y -= _CloudsHeight;
                #endif
                #if !STARS_OFF
                    OUT.twinklingPosition = RotateAroundYInDegrees(float4(dir, 1.0), _Time.y * _StarsTwinklingSpeed);
                #endif

                return OUT;
            }

            // Fragment shader is byte-for-byte identical to LowPolySkybox
